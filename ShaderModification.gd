...
func _process(delta: float):
    ...
    # VIEW_MATRIX = VIEW_MATRIX.rotated(Vector3(0.0,1.0,0.0), PI)
    ...
    var INVERSE_CAMERA_PROJECTION = main_camera.get_camera_projection().inverse()
    main_camera.get_camera_projection()
    ...
    material.set_shader_parameter("INVERSE_VIEW_MATRIX", INVERSE_VIEW_MATRIX)

    material.set_shader_parameter("test_distance", slider.value)
